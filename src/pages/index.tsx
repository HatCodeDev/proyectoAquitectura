...
import { Button, Alert, Textarea } from "@heroui/react";
  ...
  return (
    <DefaultLayout>
      <section className="flex flex-col items-center justify-center gap-6 py-8 md:py-10">
        ...
        {/* Formulario con botón Interpretar con estilo shadow */}
        <div className="mt-8 w-full flex justify-center px-4">
          <MipsInputForm
            onSubmit={processInstruction}
            isLoading={isLoading}
          />
        </div>
        ...
        <div className="mt-8 w-full max-w-xl px-4 min-h-[100px]">
          ...
          {error && !isLoading && (
            <Alert
              color="danger"
              title={`Error: ${error.message}${error.stage ? ` (Etapa: ${error.stage})` : ""}`}
            />
          )}
